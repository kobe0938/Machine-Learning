...
def softmax(x):
    ...
    return x_exp / np.expand_dims(denominator, axis=-1)
    # *** END CODE HERE ***
...
def sigmoid(x):
    ...
    x_pos = (x >= 0) * x
    x_neg = (x < 0) * x
    return 1 / (1 + np.exp(-x_pos)) + np.exp(x_neg) / (1 + np.exp(x_neg)) - np.ones(x.shape) / 2
    # *** END CODE HERE ***
...
def get_initial_params(input_size, num_hidden, num_output):
    ...
    # *** START CODE HERE ***
    W1, W2 = np.random.normal(size=(num_hidden, input_size)), np.random.normal(size=(num_output, num_hidden))
    b1, b2 = np.zeros(num_hidden), np.zeros(num_output)
...
def forward_prop(data, labels, params):
    ...
    hidden_layer = sigmoid(np.dot(data, np.transpose(W1)) + b1)
    output_layer = softmax(np.dot(hidden_layer, np.transpose(W2)) + b2)
    avg_loss = - np.sum(labels * np.log(output_layer)) / data.shape[0]
    return hidden_layer, output_layer, avg_loss
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    # *** START CODE HERE ***
    len = data.shape[0]
    hidden_layer, output_layer, avg_loss = forward_prop_func(data, labels, params)
    gradient_z2 = output_layer - labels
    gradient_b2 = gradient_z2
    gradient_W2 = np.matmul(gradient_z2.reshape((len, -1, 1)), hidden_layer.reshape((len, 1, -1)))
    gradient_z1 = gradient_z2 @ params["W2"]
    gradient_b1 = gradient_z1 * hidden_layer * (1 - hidden_layer)
    gradient_W1 = np.matmul(gradient_b1.reshape((len, -1, 1)), data.reshape((len, 1, -1)))
    gradient = {}
    gradient["W1"] = np.mean(gradient_W1, axis=0)
    gradient["b1"] = np.mean(gradient_b1, axis=0)
    gradient["W2"] = np.mean(gradient_W2, axis=0)
    gradient["b2"] = np.mean(gradient_b2, axis=0)
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func,
    ...
    # *** START CODE HERE ***
    batch_data = train_data.reshape((-1, batch_size, train_data.shape[1]))
    batch_labels = train_labels.reshape((-1, batch_size, train_labels.shape[1]))
    for i in range(len(batch_data)):
        d, l = batch_data[i], batch_labels[i]
        gradient = backward_prop_func(d, l, params, forward_prop_func)
        for j in gradient:
...
def nn_train(
    ...
    for epoch in range(num_epochs):
        ...
        print(accuracy_train[-1], accuracy_dev[-1])  # for debugging
