...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    for iteration in range(max_iter):
        new_assign = np.zeros(shape=[num_clusters, 1])
        for x in range(H):
            for y in range(W):
                distance = np.zeros(shape=[num_clusters, 1])
                for i in range(num_clusters):
                    difference = centroids[i, :] - image[x, y, :]
                    distance[i] = np.dot(np.transpose(difference), difference)
                centroid_i = distance.argmin()
                new_assign[centroid_i] += 1
                new_centroids[centroid_i, :] += image[x, y, :]
        for j in range(num_clusters):
            if new_assign[j] > 0:
                new_centroids[j, :] = new_centroids[j, :] / new_assign[j]
    # *** END YOUR CODE ***
    ...
    for i in range(H):
        for j in range(W):
            ...
            for index in range(num_clusters):
                difference = centroids[index, :] - image[i, j, :]
                distance[index] = np.dot(np.transpose(difference), difference)
            image[i, j, :] = centroids[distance.argmin()]
    # *** END YOUR CODE ***
    return image
